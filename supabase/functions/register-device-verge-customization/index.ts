...
serve(async (req) => {
  ...
  try {
    ...
    const supabase = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_ANON_KEY", { db: { schema: "verge_customization" } })!
    );
    ...
    const supabaseAdmin = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY", { db: { schema: "verge_customization" } })!
    );
