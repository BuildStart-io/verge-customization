...
serve(async (req) => {
  ...
  try {
    ...
    const supabase = createClient(supabaseUrl, supabaseServiceKey, { db: { schema: "verge_customization" } });
    ...
    const productCatalog = products.map(p => {
      ...
      if (p.size_chart_url) {
        line += ` | Size Chart Image: ${p.size_chart_url}`;
      }
      if (p.variations && Array.isArray(p.variations) && p.variations.length > 0) {
        const varLines = p.variations.map((v: any) => {
          const opts = v.options?.map((o: any) => {
            ...
            if (o.imageUrl) optStr += ` (Image: ${o.imageUrl})`;
...
- Your visible reply must ALWAYS be plain, human-readable text only.`;
            ...
            if (orderError) {
              ...
              try {
                ...
                if (ownerPhone) {
                  ...
                  const sendNotif = await fetch(`${supabaseUrl}/functions/v1/send-whatsapp-verge-customization`, {
                    method: "POST",
                    headers: {
    ...
    // Extract NOTIFY_OWNER if present
